...
ESplotDur <- ggplot(mk212_during.nooutlier, aes(x = factor(Dose), y = AbsoluteTurnAngle, colour = Treatment)) + geom_boxplot(outlier.shape = NA) + geom_point(data = ESInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + ylim(0, 90) + geom_line(data = ESInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_jitter(alpha = 0.5, position = position_jitter(width = .05)) + geom_hline(yintercept = 120, linetype = "dashed", color = "gray") + labs(x = "MK-212 dose (mg/kg)", y = "Erratic swimming \n(Abs. Turn Angle)", color = "Condition") + scale_colour_brewer(palette = "Set1") + theme_bw() + annotate(geom = "text", x = 0.8, y = 35, label = "b", size = 4) + annotate(geom = "text", x = 1.2, y = 60, label = "a", size = 4) +  annotate(geom = "text", x = 1.8, y = 60, label = "a", size = 4) + annotate(geom = "text", x = 2.2, y = 32, label = "b", size = 4) + annotate(geom = "text", x = 2.8, y = 48, label = "a", size = 4) + annotate(geom = "text", x = 3.2, y = 35, label = "b", size = 4) + theme(text = element_text((size = 14))) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"))
```
