...
* Run ANOVA for main and interaction effects on 2-way ANOVA, for behavior during exposure

1) Time on top
1.1) Analysis
```{r}
anova.tt.during <- aov(Time.Top ~ Treatment + Dose + Treatment:Dose, data = way_during)
summary(anova.tt.during)
anova_stats(anova.tt.during)
PTTT <- TukeyHSD(anova.tt.during)
multcompLetters(PTTT$`Treatment:Dose`[,'p adj'])
```

1.2) Graphics
```{r}
TTInt <- ddply(way_during, .(Dose, Treatment), summarise, val = mean(Time.Top))
TTdurplot <- ggplot(way_during, aes(x = factor(Dose), y = Time.Top, colour = Treatment)) + geom_boxplot(outlier.shape = NA) + geom_point(shape = 18, size = 4, data = TTInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + ylim(0, 360) + geom_line(data = TTInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_jitter(position = "dodge", alpha = 0.3) + geom_hline(yintercept = 120, linetype = "dashed", color = "gray") + labs(x = "Dose", y = "Time on top (s)", color = "Treatment") + scale_colour_brewer(palette = "Set1") + theme_bw() + annotate("text", x = 0.8, y = 52, label = "b") + annotate("text", x = 1.2, y = 30, label = "ab") + annotate("text", x = 1.8, y = 60, label = "ab") + annotate("text", x = 2.2, y = 28, label = "a")
```

2) Time on bottom
2.1) Analysis
```{r}
anova.tb.during <- aov(Time.Bottom ~ Treatment + Dose + Treatment:Drug, data = way_during)
summary(anova.tb.during)
anova_stats(anova.tb.during)
PTTB <- TukeyHSD(anova.tb.during)
multcompLetters(PTTB$`Treatment:Dose`[,'p adj'])
```

2.2) Graphics
```{r}
TBInt <- ddply(way_during, .(Dose, Treatment), summarise, val = mean(Time.Bottom))
TBdurplot <- ggplot(way_during, aes(x = factor(Dose), y = Time.Bottom, colour = Treatment)) + geom_boxplot(outlier.shape = NA) + geom_point(shape = 18, size = 4, data = TBInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + ylim(0, 370) + geom_line(data = TBInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_jitter(position = "dodge", alpha = 0.3) + geom_hline(yintercept = 120, linetype = "dashed", color = "gray") + labs(x = "Dose", y = "Time on bottom third (s)", color = "Treatment") + scale_colour_brewer(palette = "Set1") + theme_bw() + annotate("text", x = 0.8, y = 330, label = "b") + annotate("text", x = 1.2, y = 370, label = "a") + annotate("text", x = 1.8, y = 370, label = "b") + annotate("text", x = 2.2, y = 370, label = "b")
```

3) Erratic swimming
3.1) Analysis
```{r}
anova.es.during <- aov(AbsoluteTurnAngle ~ Treatment + Dose + Treatment:Dose, data = way_during)
summary(anova.es.during)
anova_stats(anova.es.during)
PTES <- TukeyHSD(anova.es.during)
multcompLetters(PTES$`Treatment:Dose`[,'p adj'])
```

3.2) Graphics
```{r}
ESInt <- ddply(way_during, .(Dose, Treatment), summarise, val = mean(AbsoluteTurnAngle))
ESdurplot <- ggplot(way_during, aes(x = factor(Dose), y = AbsoluteTurnAngle, colour = Treatment)) + geom_boxplot(outlier.shape = NA) + geom_point(shape = 18, size = 4, data = ESInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_line(data = ESInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_jitter(position = "dodge", alpha = 0.3) + labs(x = "Drug", y = "Absolute turn angle (º)", color = "Treatment") + scale_colour_brewer(palette = "Set1") + theme_bw() + annotate("text", x = 0.8, y = 30, label = "b") + annotate("text", x = 1.2, y = 48, label = "a") + annotate("text", x = 1.8, y = 22, label = "b") + annotate("text", x = 2.2, y = 25, label = "b")
```

4) Freezing
4.1) Analysis
```{r}
anova.fr.during <- aov(Freezing.S ~ Treatment + Dose + Treatment:Dose, data = way_during)
summary(anova.fr.during)
anova_stats(anova.fr.during)
PTFR <- TukeyHSD(anova.fr.during)
multcompLetters(PTFR$`Treatment:Dose`[,'p adj'])
```

4.2) Graphics
```{r}
FrInt <- ddply(way_during, .(Dose, Treatment), summarise, val = mean(Freezing.S))
Frdurplot <- ggplot(way_during, aes(x = factor(Dose), y = Freezing.S, colour = Treatment)) + geom_boxplot(outlier.shape = NA) + geom_point(shape = 18, size = 4, data = FrInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_line(data = FrInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_jitter(position = "dodge", alpha = 0.3) + labs(x = "Dose", y = "Freezing (s)", color = "Treatment") + scale_colour_brewer(palette = "Set1") + theme_bw() + annotate("text", x = 0.8, y = 28, label = "b") + annotate("text", x = 1.2, y = 140, label = "a") + annotate("text", x = 1.8, y = 45, label = "b") + annotate("text", x = 2.2, y = 57, label = "ab")
```

5) Swimming speed
5.1) Analysis
```{r}
anova.Sp.during <- aov(Speed ~ Treatment + Dose + Treatment:Dose, data = way_during)
summary(anova.Sp.during)
anova_stats(anova.Sp.during)
PTSP <- TukeyHSD(anova.Sp.during)
multcompLetters(PTSP$`Treatment:Dose`[,'p adj'])
```
