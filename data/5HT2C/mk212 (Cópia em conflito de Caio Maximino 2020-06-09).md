...
ESplotAfter <- ggplot(mk212_after.nooutlier, aes(x = factor(Dose), y = AbsoluteTurnAngle, colour = Treatment)) +  ylim(0, 90) + geom_boxplot(outlier.shape = NA) + geom_point(data = ESInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_line(data = ESInt, aes(y = val, group = Treatment), position = position_dodge(width = 0.75)) + geom_jitter(position = "dodge", alpha = 0.3) + labs(x = "Dose", y = "Erratic swimming (Abs. Turn Angle)", color = "Treatment") + scale_colour_brewer(palette = "Set1") + theme_bw() + annotate(geom = "text", x = 0.8, y = 35, label = "b") + annotate(geom = "text", x = 1.2, y = 65, label = "a") +  annotate(geom = "text", x = 1.8, y = 55, label = "a") + annotate(geom = "text", x = 2.2, y = 33, label = "b") + annotate(geom = "text", x = 2.8, y = 48, label = "a") + annotate(geom = "text", x = 3.2, y = 35, label = "b")
```
...
3.2) Graphics
```{r}
ESInt <- ddply(mk212_after.nooutlier, .(Dose, Treatment), summarise, val = mean(Erratic.Swimming.N))
...
4.2) Graphics
```{r}
FrSInt <- ddply(mk212_after.nooutlier, .(Dose, Treatment), summarise, val = mean(Freezing.N))
